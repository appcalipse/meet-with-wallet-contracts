...
describe("MWWSubscription", async () => {
  ...
  it("should have second account as registrar", async () => {
    const _registar = await instance.getRegisterContract()
  ...
  it("should create subscription", async () => {
    await instance.connect(registrar).subscribe(0, user.address , 100, "mww.eth", "")
  ...
  it("should emit subscription event", async () => {
    await expect(instance.connect(registrar).subscribe(0, user.address , 100, "mww.eth", "")).to
    .emit(instance, "MWWSubscribed")
    .withArgs(user.address, 0, 100, "mww.eth")
  });

  it("should fail to subscribe", async () => {
    await expect(instance.connect(user).subscribe(0, user.address , 100, "mww.eth", "")).to.be.revertedWith("Only the register can call this")
    await expect(instance.connect(user).addSubscription(0, user.address , 100, "mww.eth", "")).to.be.revertedWith("Only admin can do it")
  });
  ...
  it("Fetching domains from account works", async () => {
    await instance.connect(registrar).subscribe(0, user.address , 100, "mww.eth", "")
    await instance.connect(registrar).subscribe(0, user.address , 100, "mww2.eth", "")
  ...
  it("Should extend subscription", async () => {
   await instance.connect(registrar).subscribe(0, user.address, 300, "mww.eth", "")
    const sub = await instance.getSubscription("mww.eth")
   await instance.connect(registrar).subscribe(0, user.address, 300, "mww.eth", "")
  ...
  it("Should fail to extend subscription with different plan or user", async () => {
    await instance.connect(registrar).subscribe(0, user.address , 100, "mww.eth", "")
    await expect(instance.connect(registrar).subscribe(1, user.address , 500, "mww.eth", "")).to.be.revertedWith("Domain registered with another plan")
    await expect(instance.connect(registrar).subscribe(0, newAdmin.address , 500, "mww.eth", "")).to.be.revertedWith("Domain registered for someone else")

    await blockChainDelay(1000);
    
    await instance.connect(registrar).subscribe(0, newAdmin.address , 100, "mww.eth", "")
  })
  ...
  it("should not be able to change domain on subscription it doesn't own", async () => {
    await instance.connect(registrar).subscribe(0, user.address, 100, "mww.eth", "")
    await instance.connect(registrar).subscribe(0, newAdmin.address, 100, "mww2.eth", "")
  ...
  it("should change ipfs hash", async () => {
    await instance.connect(registrar).subscribe(0, user.address, 100, "mww.eth", "hash1")
    const sub = await instance.getSubscription("mww.eth")
