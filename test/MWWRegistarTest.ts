...
describe("MWWRegistar", async () => {
  ...
  it("should purchase with MATIC", async () => {
    ...
    await instance.connect(newAdmin).purchaseWithNative(0, user.address, YEAR_IN_SECONDS, "look at me", "hash", {value: planPriceInWei})
  ...
  it("should change plan price", async () => {
    await instance.addPlan("PRO", 30, 0)
    const plans = await instance.getAvailablePlans()
    expect(plans[0].planId).to.eq(0)
    expect(plans[0].usdPrice).to.eq(BigNumber.from(30))
    await instance.removePlan(0)
    await instance.addPlan("PRO", 50, 0)
    const plans2 = await instance.getAvailablePlans()
    expect(plans2[0].planId).to.eq(0)
    expect(plans2[0].usdPrice).to.eq(BigNumber.from(50))
  });

  it("should fail to purchase without approving token", async () => {
    await instance.addPlan("PRO", 30, 0)
    await expect(instance.purchaseWithToken(usdc.address, 0, user.address, YEAR_IN_SECONDS, "look at me", "hash")).to.be.revertedWith("Allowance lower than needed")
  });
  ...
  it("should purchase with USDC", async () => {
    ...
    await instance.addPlan("PRO", 30, 0)
    await instance.connect(newAdmin).purchaseWithToken(usdc.address, 0, user.address, YEAR_IN_SECONDS, "look at me", "hash")
  ...
  it("should purchase with USDC for other durations", async () => {
    ...
    await instance.addPlan("PRO", 30, 0)
    await instance.connect(user).purchaseWithToken(usdc.address, 0, user.address, 2*YEAR_IN_SECONDS, "look at me", "hash")
    ...
    await instance.connect(user).purchaseWithToken(usdc.address, 0, user.address, Math.round(YEAR_IN_SECONDS/3), "look at me2", "hash")
    const newUserBalance = await usdc.balanceOf(user.address)
    expect(newUserBalance).to.be.closeTo(BigNumber.from(30*10**6),10)
  });

  it("should fail to buy with non accepted token", async () => { 
    await expect(instance.purchaseWithToken(dai.address, 0, user.address, YEAR_IN_SECONDS, "look at me", "hash")).to.be.revertedWith("Token not accepted")
  });

  it("should remove acceptable token", async () => {
    await instance.removeAcceptableToken(usdc.address)
    await expect(instance.purchaseWithToken(usdc.address, 0, user.address, YEAR_IN_SECONDS, "look at me", "hash")).to.be.revertedWith("Token not accepted")
  });
  ...
  it("should add acceptable token and pay with it", async () => { 
    ...
    await instance.connect(user).purchaseWithToken(dai.address, 0, user.address, YEAR_IN_SECONDS, "look at me", "hash")
  });
  ...
  it("should withdraw", async () => {
    ...
    await instance.connect(user).purchaseWithToken(usdc.address, 0, user.address, YEAR_IN_SECONDS, "look at me", "hash")
    await instance.connect(user).purchaseWithToken(dai.address, 0, user.address, YEAR_IN_SECONDS, "look at me", "hash")
    const planPriceInWei = (await instance.getNativeConvertedValue(30))
    await instance.connect(newAdmin).purchaseWithNative(0, user.address, YEAR_IN_SECONDS, "look at me", "hash", {value: planPriceInWei})
  ...
  it("should fail to withdraw", async () => {
    await expect(instance.withdraw(ethers.constants.AddressZero, 100, user.address)).to.be.revertedWith("Not enough funds to be withdrawn")
